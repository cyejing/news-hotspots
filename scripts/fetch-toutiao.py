...
职责：
- 调用 `bb-browser site toutiao/hot` 获取热榜
- 将热榜条目标准化为统一 `articles`
- 将抓取耗时、失败信息和慢请求统计写入 `*.meta.json`

执行逻辑：
1. 加载 runtime 配置
2. 顺序执行 Toutiao 抓取并遵守 cooldown
3. 将结果转换为统一 article 结构
4. 输出结果 JSON 与 sidecar meta JSON

输出文件职责：
- `<step>.json`
  只保存抓到的 Toutiao article
- `<step>.meta.json`
  只保存 step 级诊断，不承担结果数据表达职责
"""
...
def apply_runtime_config(defaults_dir: Path, config_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    fetch_config = runtime.get("fetch", {}).get("toutiao", {})
...
Examples:
    ...
    parser.add_argument("--limit", type=int, default=None, help="Number of Toutiao hot items to request")
...
def main() -> int:
    ...
    try:
        ...
        output = {"generated": data.get("generated"), "source_type": "toutiao", "articles": source_result.get("articles", [])}
